...
class Spectra:
    ...
    def read_as_chn(self, filename):
        ...
        self.spectra = numpy.frombuffer(self.data[struct.calcsize(unpack_str):struct.calcsize(unpack_str) + 4 * self.channels], dtype = dt)
    ...
    def extract_integer_spect(self):
        ...
        return numpy.frombuffer(self.data[(SPCTRP - 1) * 128: (SPCTRP - 1 + SPCRCN) * 128], dtype = dt)

    def extract_acq_inf_rec(self):
        ACQIRP = self.meta_data["ACQIRP"]
        relevant_str = self.data[(ACQIRP - 1) * 128: (ACQIRP - 1) * 128 + 90]
        self.short_file_name = relevant_str[0:16].decode()
        self.date_str = relevant_str[16:27].decode()
        self.time_str = relevant_str[28:38].decode()
        self.live_time_str = relevant_str[38:48].decode()
        self.real_time_str = relevant_str[48:58].decode()
    ...
    def show_spectra(self):
        ...
        ax1.plot(self.energy_calibration, self.spectra, lw=2, color="blue")
